...
def setupMotorIO():
    ...
    PWM_CONTROL_RIGHT = GPIO.PWM(MOTOR_RIGHT_ENABLE1, 100)  # Initial Freq 60
    PWM_CONTROL_LEFT = GPIO.PWM(MOTOR_LEFT_ENABLE2, 100)  # Initial Freq 60
...
def act_optimised():
    ...
    # Map the decision into an action in our optimized function
    action_map = {
        "straight": straight,
        ...
        "right": turn_right,
        "leftright": turn_left,
        "leftstraight": turn_left,
        "rightstraight": straight,
        "leftrightstraight": turn_left,
        "dead_end": turn_around,
        "stay": stay_put
    }
    
    # If there's a stored action, use it
    if len(ans) > 0:
        action = ans[0]
        ans = ans[1:]
        ...
        if action == "L":
            ...
            unmapped()  # Add any additional actions you may want to handle
        return

    # If there's no stored action, revert to the original act() logic
